(() => {
  ...
  function showInventoryMovements(product) {
    ...
    dialog.innerHTML=`<header><div><h2 id="movementTitle">Stock Movements</h2><p>${escapeHtml(product.name)} · ${escapeHtml(product.sku || '')}</p></div><button type="button" data-close>Close</button></header>
      <form class="movement-filters"><label>From<input type="date" name="from_date"></label><label>To<input type="date" name="to_date"></label><label>Type<select name="movement_type"><option value="">All movements</option><option value="in">Stock In</option><option value="out">Stock Out</option><option value="adjustment">Adjustment</option><option value="sale">Sale</option><option value="refund">Refund</option><option value="transfer">Transfer</option></select></label><button type="submit">Apply filters</button></form>
  ...
  async function openTouchReceipt(id) {
    ...
    try {
      ...
      detail.innerHTML += `<div class="receipt-detail-actions">${completed && !credit ? '<button type="button" data-refund-open>Refund</button>' : ''}<button type="button" data-reprint-receipt>Print Receipt</button></div>`;
      if (completed && !credit) {
        detail.innerHTML += `<div class="receipt-refund"><form data-refund-form hidden><strong>Refund ${money(r.total)} Ks?</strong><p>This refunds the entire receipt and restores its stock.</p><label>Reason<input name="reason" required maxlength="500" placeholder="Customer return"></label><div><button type="submit" data-refund-submit>Confirm refund</button><button type="button" data-refund-cancel>Cancel</button></div><small data-refund-error role="alert"></small></form></div>`;
