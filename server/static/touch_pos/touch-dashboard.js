(() => {
  ...
  const selectedBreakdowns={sales:'top_items',expenses:'expense_items'};
  ...
  function fillBreakdown(panel, data, key) {
    ...
    const maxAmount=Math.max(1,...rows.map(row=>Number(key==='discount_sales'?row.discount||0:row.total||0)));
    const total=rows.reduce((sum,row)=>sum+Number(key==='discount_sales' ? row.discount || 0 : row.total || 0),0);
    const unavailable=key==='wholesale_sales' && data.wholesale_available===false;
    const totalLabel=key==='payment_sales' ? 'Total sales' : key==='discount_sales' ? 'Discount total · shown receipts' : 'Total · shown records';
    const totalNote=key==='payment_sales' ? 'All payment types in the selected period' : `${rows.length} records shown · Top 10 only`;
    const content=panel.querySelector('[data-breakdown-content]');
    content.innerHTML=`<div class="dashboard-breakdown-total"><div><span>${totalLabel}</span><small>${totalNote}</small></div><strong>${unavailable ? 'Unavailable' : `${money(total)} Ks`}</strong></div><p class="dashboard-breakdown-note">${key==='expense_groups'?'Top 10 expense categories by amount.':note}</p>${rows.length ? rows.map(r=>`<article class="dashboard-breakdown-row" style="--row-share:${Math.max(0,Math.min(100,Number(r.discount!==undefined?r.discount:r.total)/maxAmount*100))}%"><span class="dashboard-row-icon" aria-hidden="true"><img src="/assets/icons/receipt.svg" alt=""></span><span class="dashboard-row-main">${ctx.escapeHtml(r.label)}<small>${r.qty!==undefined ? `${money(r.qty)} units` : r.discount!==undefined ? `Sale total: ${money(r.total)} Ks` : `${money(r.count)} records`}</small></span><strong>${money(r.discount!==undefined?r.discount:r.total)} Ks</strong></article>`).join('') : `<p>${key==='wholesale_sales' && data.wholesale_available===false ? 'Wholesale history is not available in this database.' : 'No records for this period.'}</p>`}`;
  ...
  function render(data) {
    ...
    root.querySelector('[data-results]').innerHTML=`<div id="dashboardOverview" class="dashboard-section-title"><h3>Period overview</h3><span>${ctx.escapeHtml(p.from_date || '')} – ${ctx.escapeHtml(p.to_date || '')}</span></div><div class="dashboard-cards">${cards.map(card).join('')}</div><section class="panel dashboard-summary"><h3>Sales summary</h3><div><span>Sales including refunded receipts</span><strong>${money(p.gross_sales)} Ks</strong></div><div><span>Refunded receipts</span><strong>${money(p.refunds)} Ks</strong></div><div><span>Average completed sale</span><strong>${money(p.transactions ? p.sales/p.transactions : 0)} Ks</strong></div><small>Sales use the receipt date; expenses use the expense date. Refunded receipts are excluded from completed sales.</small></section><div id="dashboardBreakdowns" class="dashboard-columns">${breakdownTabs(data,'sales')}${breakdownTabs(data,'expenses')}</div><div id="dashboardCredit" class="dashboard-section-title"><h3>Credit & outstanding</h3><span>All dates · Current balances</span></div><div class="dashboard-cards dashboard-credit-cards">${[['Credit issued',c.total,`${money(c.accounts)} non-cancelled credit accounts`,'credit'],['Credit paid',c.paid,'Payments recorded against credit accounts','sales'],['Outstanding',c.balance,'Remaining customer credit balance','discount']].map(card).join('')}</div>${creditPanel(data)}`;
    root.querySelectorAll('[data-breakdown]').forEach(panel=>{
      const buttons=panel.querySelectorAll('[data-breakdown-key]');
      const kind=panel.dataset.breakdown;
      fillBreakdown(panel,data,selectedBreakdowns[kind]);
      buttons.forEach(button=>button.onclick=()=>{selectedBreakdowns[kind]=button.dataset.breakdownKey;fillBreakdown(panel,data,button.dataset.breakdownKey);});
    });
  }
  async function load() {
    ...
    root.querySelector('[data-results]').innerHTML='<div class="dashboard-loading" aria-hidden="true"><span></span><span></span><span></span><span></span></div>';
    ...
    } catch(error){if(request===sequence && !root.hidden){root.querySelector('[data-results]').innerHTML='';status.textContent=`Could not load dashboard: ${error.message}. Use Apply to retry.`;}}
  ...
  window.KayTouchDashboard={
    ...
    show(context){
      ...
      root.hidden=false;
      root.innerHTML=`<header class="panel dashboard-header"><div><span class="dashboard-eyebrow">BUSINESS OVERVIEW</span><h2>Dashboard</h2><p>Review sales, expenses and customer credit in one place.</p></div><button type="button" data-sales data-icon="point_of_sale">Back to Sales</button></header><nav class="dashboard-jump" aria-label="Dashboard sections"><a href="#dashboardOverview">Overview</a><a href="#dashboardBreakdowns">Sales & expenses</a><a href="#dashboardCredit">Credit & outstanding</a></nav><form class="panel dashboard-filters"><label>From<input name="from" type="date" required></label><label>To<input name="to" type="date" required></label><button type="submit" class="sign-in" data-icon="check_circle">Apply</button><button type="button" data-period="today">Today</button><button type="button" data-period="week">Last 7 days</button><button type="button" data-period="month">This month</button></form><p data-status role="status" aria-live="polite"></p><div data-results></div>`;
      ...
      root.querySelectorAll('[data-period]').forEach(button=>button.onclick=()=>{const now=new Date();form.elements.to.value=date(now);if(button.dataset.period==='month')now.setDate(1);if(button.dataset.period==='week')now.setDate(now.getDate()-6);form.elements.from.value=date(now);load();});
      load();
    }
  };
})();
