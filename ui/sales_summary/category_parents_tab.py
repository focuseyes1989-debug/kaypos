# ui/sales_summary/category_parents_tab.py
"""
Sales Summary by Category Parent (Parent Category)
Shows sales grouped by parent category (top-level categories)
"""
...
class CategoryParentsTab(QWidget):
    """Sales summary by parent category (top-level categories)"""
    ...
    def __init__(self, parent=None):
        ...
        self.search_widget = ModernSearchWidget("Search parent category name...")
    ...
    def _display_data(self, rows):
        ...
        for row_data in rows:
            ...
            parent_name = row_data[0] if row_data[0] else "No Parent"
            ...
            self.table.setItem(r, 0, QTableWidgetItem(parent_name))
        ...
        if lang_code == "my":
            self.table.setHorizontalHeaderLabels([
                "မိဘအမျိုးအစား", "ရောင်းရသည့်အရေအတွက်",
            ...
            self.table.setHorizontalHeaderLabels([
                "Parent Category", "Items Sold", "Gross Sales", "Net Sales",
                "Discount", "Cost of Goods", "Gross Profit", "Progress"
            ])
    
    def load(self, from_date, to_date):
        """
        Load sales data grouped by parent category.
        Shows top-level categories (categories with no parent).
        """
        conn = connect_db()
        cursor = conn.cursor()
    ...
    def retranslateUi(self):
        ...
        if hasattr(self.search_widget, 'set_placeholder_text'):
            if lang_code == "my":
                self.search_widget.set_placeholder_text("မိဘအမျိုးအစား ရှာရန်...")
            else:
                self.search_widget.set_placeholder_text("Search parent category name...")
