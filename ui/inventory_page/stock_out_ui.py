# ui/inventory_page/stock_out_ui.py
...
class StockOutUI:
    """UI setup class for StockOutDialog - Theme-aware with SVG Icons"""
    
    def __init__(self):
        self.stock_out_no = None
    ...
    def _setup_left_panel(self, dialog, colors):
        ...
        dialog.so_product = self._create_combobox(colors)
        dialog.so_product.setMinimumWidth(200)
        product_layout.addWidget(dialog.so_product)
    ...
    def _setup_right_panel(self, dialog, colors):
        ...
        dialog.product_details_label.setFixedHeight(70)
        ...
        dialog.product_details_label.setAlignment(Qt.AlignmentFlag.AlignTop)
    ...
    def _setup_buttons(self, dialog, parent_layout, colors):
        ...
        dialog.btn_cancel.setAutoExclusive(False)
    ...
    def _store_references(self, dialog):
        ...
        dialog.stock_out_no = self.stock_out_no
    ...
    def _get_stock_label_style(self, colors, is_dark):
        if is_dark:
            return f"""
                QLabel {{
                    font-weight: 600;
    ...
    def _create_reason_combobox(self, colors):
        combo = QComboBox()
        combo.addItems(["Sale", "Damage", "Transfer", "Other"])
        combo.setStyleSheet(self._get_combobox_style(colors))
        return combo
    ...
    def _update_input_styles(self, dialog, colors):
        ...
        # Combo boxes
        if hasattr(dialog, 'so_product'):
            dialog.so_product.setStyleSheet(self._get_combobox_style(colors))
        if hasattr(dialog, 'so_customer'):
            dialog.so_customer.setStyleSheet(self._get_combobox_style(colors))
        if hasattr(dialog, 'so_reason'):
            dialog.so_reason.setStyleSheet(self._get_combobox_style(colors))
        if hasattr(dialog, 'so_location'):
            dialog.so_location.setStyleSheet(self._get_combobox_style(colors))
        
        # Spin box
        if hasattr(dialog, 'so_qty'):
            dialog.so_qty.setStyleSheet(self._get_spinbox_style(colors))
        
        # Date edit
        if hasattr(dialog, 'so_date'):
            dialog.so_date.setStyleSheet(self._get_input_style(colors))
        
        # Text edit
        if hasattr(dialog, 'so_notes'):
            dialog.so_notes.setStyleSheet(self._get_input_style(colors))
        
        # Line edits
        if hasattr(dialog, 'so_reference'):
            dialog.so_reference.setStyleSheet(self._get_input_style(colors))
        if hasattr(dialog, 'so_issued_by'):
            dialog.so_issued_by.setStyleSheet(self._get_input_style(colors))
        
        # Current stock label
        if hasattr(dialog, 'current_stock_label'):
            dialog.current_stock_label.setStyleSheet(self._get_stock_label_style(colors, self._is_dark))