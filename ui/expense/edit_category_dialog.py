# ui/expense/edit_category_dialog.py
"""
...
class EditCategoryDialog(QDialog):
    """Dialog for editing an expense category"""
    
    def __init__(self, category_id, old_name, parent=None):
        super().__init__(parent)
        self.category_id = category_id
        self.old_name = old_name
        self._category_name = None
        
        self.setWindowTitle("Edit Category" if lang.get_current() != "my" else "အမျိုးအစားပြင်ဆင်ရန်")
    ...
    def _setup_ui(self):
        ...
        # Title
        self.title_label = QLabel("Edit Category" if lang.get_current() != "my" else "အမျိုးအစားပြင်ဆင်ရန်")
        ...
        # Label
        self.label = QLabel("New Category Name" if lang.get_current() != "my" else "အမျိုးအစားအမည်အသစ်")
        ...
        self.input_name.setText(self.old_name)
        ...
        # Helper text
        self.helper_text = QLabel("Press Enter to save or click Save button" if lang.get_current() != "my" else "Enter နှိပ်ပြီး သိမ်းပါ သို့မဟုတ် Save ခလုတ်ကို နှိပ်ပါ")
        ...
        # ✅ Save button - Primary (White icon in both themes)
        self.btn_ok = ModernButton(
            " Save" if lang.get_current() != "my" else " သိမ်းမည်", 
            ModernButton.PRIMARY
        )
        self.btn_ok.set_icon("save", size=(14, 14))
    ...
    def retranslate_ui(self):
        ...
        if lang_code == "my":
            self.setWindowTitle("အမျိုးအစားပြင်ဆင်ရန်")
            self.title_label.setText("အမျိုးအစားပြင်ဆင်ရန်")
            self.label.setText("အမျိုးအစားအမည်အသစ်")
            self.input_name.setPlaceholderText("အမျိုးအစားအမည်အသစ် ထည့်ပါ...")
            self.helper_text.setText("Enter နှိပ်ပြီး သိမ်းပါ သို့မဟုတ် Save ခလုတ်ကို နှိပ်ပါ")
            ...
            self.label.setText("New Category Name")
            self.input_name.setPlaceholderText("Enter new category name...")
            self.helper_text.setText("Press Enter to save or click Save button")
    ...
    def _accept(self):
        ...
        conn = connect_db()
        cursor = conn.cursor()
        cursor.execute("SELECT id FROM expense_categories WHERE name = ? AND id != ?", (name, self.category_id))
