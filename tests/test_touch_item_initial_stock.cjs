...
c.editingProduct={id:1};fields['#itemSoldBy'].value='Each';c.updateItemMode();assert.equal(fields['#itemCost'].label.hidden,false);assert.equal(fields['#itemStock'].label.hidden,false);assert.equal(c.itemPayload().cost,900);assert.equal(c.itemPayload().stock,12);
