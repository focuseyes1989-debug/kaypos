...
const ctx={setupInventoryActions:()=>{},document:{querySelector:node,querySelectorAll:()=>[]},window:{confirm:()=>confirm,matchMedia:()=>({matches:false})},URLSearchParams,
...
await ctx.openInventoryProduct(1);assert.match(node('#inventoryDetail').innerHTML,/No expiry/);assert.equal(node('location').disabled,false);node('variant_id').value='9';node('quantity').value='3';node('cost').value='120';
