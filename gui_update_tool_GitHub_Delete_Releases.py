...
def resolve_release_steps(launcher_only: bool) -> list[str]:
    """Return the build steps for the selected release workflow."""
    if launcher_only:
        return ["build_launcher"]
    return ["build_exe", "build_launcher"]


def detect_default_version(project_root: Path | None = None) -> str:
    root = project_root or PROJECT_ROOT
    for path in (
        root / "update_build" / "version.json",
        root / "version.json",
        root / "update_server" / "version.json",
    ):
        try:
...
def resolve_repo(value: str | None = None) -> str:
    candidate = (value or "").strip()
    if candidate:
        return candidate
    env_value = os.getenv("GITHUB_REPO", "").strip()
    return env_value or DEFAULT_REPO


def resolve_token(value: str | None = None) -> str:
    candidate = (value or "").strip()
    if candidate:
        return candidate
...
class ReleaseToolGUI:
    def __init__(self, root):
        ...
        self.version_var = tk.StringVar(value=detect_default_version())
        self.token_var = tk.StringVar(value=resolve_token())
        self.repo_var = tk.StringVar(value=resolve_repo())
        self.recreate_release_var = tk.BooleanVar(value=False)
        self.launcher_only_var = tk.BooleanVar(value=True)
    ...
    def setup_ui(self):
        ...
        self.launcher_only_check = ttk.Checkbutton(
            settings,
            text="Launcher-only release (skip main EXE build)",
            variable=self.launcher_only_var,
        )
        self.launcher_only_check.grid(row=3, column=1, columnspan=3, sticky="w", pady=(6, 0))
        ...
        self.delete_release_btn = ttk.Button(
            actions,
            text="Delete All GitHub Releases",
            command=self.delete_all_releases
        )
        self.delete_release_btn.grid(
            row=2,
            column=0,
            columnspan=5,
            sticky="ew",
    ...
    def validate_repo(self):
        repo = resolve_repo(self.repo_var.get())
    ...
    def run_upload_update(self):
        ...
        token = resolve_token(self.token_var.get())
        if not token:
            messagebox.showerror(
                "GitHub Token Required",
                "Enter a GitHub token or set GITHUB_TOKEN / GITHUB_REPO.",
            )
            return
    ...
    def run_all(self):
        ...
        steps = resolve_release_steps(self.launcher_only_var.get())
        tasks = []
        for step_name in steps:
            if step_name == "build_exe":
                tasks.append(self.create_build_exe_task(version))
            elif step_name == "build_launcher":
                tasks.append(self.create_build_launcher_task(version))

        tasks.append(self.create_generate_update_task(version, repo))
        if token:
    ...
    def delete_all_releases(self):
        ...
        self.set_busy(True)
        self.queue_status("Deleting GitHub releases...")
        threading.Thread(
            target=self.delete_release_worker,
            args=(repo, token),
            daemon=True
        ).start()


    def delete_release_worker(self, repo, token):
        try:
            ...
            if not releases:
                self.queue_log("No releases found.")
                self.queue_status("No releases")
                return
            ...
            for release in releases:
                ...
                urllib.request.urlopen(delete_req).close()
                self.queue_log(f"Deleted release: {tag}")

            self.queue_log("\nAll GitHub releases deleted successfully.")
            self.queue_status("Completed")

        except Exception as exc:
            self.queue_log(f"Delete failed: {exc}")
            self.queue_status("Failed")
        finally:
            self.log_queue.put(("busy", False))
